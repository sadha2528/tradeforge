...
export function LeftToolbar() {
  ...
  return (
    <div className="w-full h-full bg-[#0a0e17] border-r border-[#1a1f2e] flex flex-col items-center py-2 space-y-1 z-20 overflow-y-auto">
      {TOOLS.map((tool) => {
          ...
          <Tooltip key={tool.id}>
            ...
            <TooltipContent side="right" className="bg-[#121828] border-[#1f2a40] text-gray-200 font-mono text-xs">
              <p className="flex items-center space-x-2">
                ...
                {tool.shortcut && <kbd className="bg-[#1a2336] px-1 py-0.2 rounded text-[10px] text-gray-400">{tool.shortcut}</kbd>}
              </p>
            </TooltipContent>
          </Tooltip>
        );
